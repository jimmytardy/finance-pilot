      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <Card className="mb-8">
          <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Tabs value={String(horizonYears)} onValueChange={(v) => setHorizonYears(Number(v) as HorizonYears)}>
              <TabsList className="h-8">
                <TabsTrigger value="10" className="px-3 text-xs">
                  {t('estimations.horizon10')}
                </TabsTrigger>
                <TabsTrigger value="20" className="px-3 text-xs">
          ...
          <CardContent>
            {noInvestmentsBoth ? (
              <p className="py-8 text-center text-sm text-muted-foreground">{t('comparaison.noInvestmentsBoth')}</p>
            ) : (
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <div className="rounded-lg border border-border bg-secondary/30 p-4">
                <p className="mb-1 text-xs text-muted-foreground">{t('comparaison.endHorizon', { name: nameA })}</p>
                <p className="font-mono text-xl font-bold">{formatCurrency(endA)}</p>
              </div>
              <div className="rounded-lg border border-border bg-secondary/30 p-4">
                <p className="mb-1 text-xs text-muted-foreground">{t('comparaison.endHorizon', { name: nameB })}</p>
                <p className="font-mono text-xl font-bold">{formatCurrency(endB)}</p>
              </div>
              <div className="rounded-lg border border-primary/25 bg-primary/5 p-4">
                <p className="mb-1 text-xs text-muted-foreground">{t('comparaison.gapTitle', { years: horizonYears })}</p>
                <p className={`font-mono text-xl font-bold ${endA - endB >= 0 ? 'text-primary' : 'text-destructive'}`}>
                  {formatCurrency(endA - endB)}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">{t('comparaison.gapHint')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10 bg-muted/95 backdrop-blur">
                  <tr className="border-b text-left">
                    ...
                    <th className="p-3 text-right font-medium">{nameA}</th>
                    <th className="p-3 text-right font-medium">{nameB}</th>
                    <th className="p-3 text-right font-medium">{t('comparaison.tableGap')}</th>
                  </tr>
                </thead>
                <tbody>
                  {chartRows.map((row) => (
                    <tr key={row.yearLabel} className="border-b border-border/60 last:border-0">
                      ...
                      <td className="p-3 text-right font-mono tabular-nums">{formatCurrency(row.portfolioA)}</td>
                      <td className="p-3 text-right font-mono tabular-nums">{formatCurrency(row.portfolioB)}</td>
                      <td
    ...
    <Card className="border-border/80">
      ...
      <CardContent className="space-y-2 text-sm">
        <div className="flex justify-between gap-2">
          <span className="shrink-0 text-muted-foreground">{t('comparaison.scenarioLabelA')}</span>
          <span className="text-right font-mono tabular-nums">{format(a)}</span>
        </div>
        <div className="flex justify-between gap-2">
          <span className="shrink-0 text-muted-foreground">{t('comparaison.scenarioLabelB')}</span>
          <span className="text-right font-mono tabular-nums">{format(b)}</span>
        </div>
      </CardContent>
    </Card>
  )
